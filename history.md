...
## 2026-05-24 — Сайдбар: починка пустого списка после редактирования

- `rebuild_sidebar()`: `gtk_widget_show_all(sidebar_scroll)` после перестроения — иначе новые строки невидимы

## 2026-05-24 — Правки сайдбара: ширина, формат ":N", пустые слоты редактируемые

- Авто-ресайз сайдбара под ширину текста через Pango layout measurement
- Цифра справа в формате `:N` (как в диалоге)
- Пустые строки не пропускаются при навигации ↑↓
- `e`/`E`/`у`/`У`/`F4` — редактирование выбранной папки из сайдбара
- `z`/`Z`/`я`/`Я` на пустом слоте — открывает диалог редактирования
- Пустые слоты показываются без текста (без "(empty)")
- `--help`: обновлён список шорткатов, убран дублирующийся раздел Dialog keys
